...
import type { OvertimeEntry, OvertimeEntryForm } from "@/types"
...
export const useOvertimeStore = defineStore("overtime", () => {
    ...
    const totalOvertimeHours = computed(() =>
        overtimeEntries.value.filter((entry) => entry.status === "APPROVED").reduce((total, entry) => total + entry.hours, 0)
    )
    ...
    const createOvertimeEntry = async (overtimeData: OvertimeEntryForm) => {
        loading.value = true
        error.value = null
    ...
    const updateOvertimeEntry = async (id: string, overtimeData: Partial<OvertimeEntryForm>) => {
        loading.value = true
        error.value = null
    ...
    const pendingOvertimeEntries = ref<OvertimeEntry[]>([])
    ...
    const approveOvertimeEntry = async (id: string) => {
        loading.value = true
        error.value = null
    ...
    const rejectOvertimeEntry = async (id: string) => {
        loading.value = true
        error.value = null
